...
4. **Persistently Update Backstory:** You MUST then call the `interactive_teacher_tool` with the `agent_name` of the failing worker (e.g., 'creator', 'auditor', 'assistant') and the `proposed_rules` you formulated. This will prompt the user to approve the update, persistently writing it to the worker's instruction file.
